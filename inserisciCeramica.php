...
<?
	if ($ok == 1){
		...
		$query="insert into ceramica (nome,indirizzo,telefono,idgruppo,colore) values(\"" . mysqli_escape_string($db,  $_REQUEST['nome']). "\",'". mysqli_escape_string($db, $_REQUEST['indirizzo'])."','".mysqli_escape_string($db, $_REQUEST['telefono'])."','". mysqli_escape_string($db, $_REQUEST['idgruppo']) ."',\"" . mysqli_escape_string($db, $_REQUEST["colore"]) . "\")";
