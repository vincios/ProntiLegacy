<?
	...
	if(isset($_GET['Dal']) && isset($_GET['Al'])){
		...
		$query = "DELETE FROM prontimarazzi WHERE eliminato=1 and data_eliminazione between '$Dal_anno-$Dal_mese-$Dal_giorno' and '$Al_anno-$Al_mese-$Al_giorno'";
	...
	elseif(isset($_GET['Giorno'])){
		...
		$query = "DELETE FROM prontimarazzi WHERE eliminato=1 and data_eliminazione='$Giorno_anno-$Giorno_mese-$Giorno_giorno'";
		mysqli_query($db, $query) or die( mysqli_error($db) );
